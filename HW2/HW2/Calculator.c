...
void subtract(char num1[], char num2[], char result[]) {
	...
	i = strlen(temp) - 1;
	while (i >= 0 && temp[i] == '0') {
		i--; // ex) temp에 7000 이 저장된 경우, 0이 아닌 수가 처음 나올 때까지 0을 없앤다.
	}
	if (i == -1) result[idx++] = '0'; // 모든 수가 0이 었을 경우, 0을 남겨야 한다.
	for (i; i >= 0; i--) {
		result[idx++] = temp[i]; // ex) 1237000 이었을 경우, 7321 순으로 result에 저장. 
	}
